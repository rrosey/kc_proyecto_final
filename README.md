...
Para llevar a cabo el scraping e ingestión de las fuentes de datos seleccionadas, se ha optado por desarrollar unos scripts en Python que almacenados en la nube y que se ejecutaran cada cierto tiempo dependiendo de la naturaleza de los datos (diariamente, semanalmente, …). Estos datos obtenidos serán preprocesados, para hacer ciertas transformaciones, limpieza y se almacenarán en el servicio de almacenamiento de Google **GCS (Google Cloud Storage)**. 
La plataforma de Google Cloud dispone de varias opciones para poder desplegar los scripts a ejecutar para llevar a cabo la ingestión.
...
¿Con qué frecuencia y dónde procesará las recomendaciones? ¿dónde se almacenarán? Probablemente, baste con actualizar las viviendas recomendadas una vez por día o incluso una vez por semana. Es decir en esta primera fase y acorde a la naturaleza de los datos el procesamiento a realizar será en batch. Además se buscaba una que fuera tolerante a fallos y escalable a grandes volumenes de datos.
Los distintos datasets ingestados en formato csv se almancenan en el Cloud Storage de Google, y mediante una tecnología como Hive se llevarán a cabo las tareas de extracción, transformación y carga. Esta tecnología permite trabajar con grandes volúmenes de datos y hacer consultas y analisis sobre ellos mediante lenguaje SQL como si se tratara de una base de datos relacional. Para ello se hará uso de dos servicios proporcionados por GCP:
-	Cloud Dataproc: Se trata de un servicio administrado para crear clústeres de Spark y Hadoop y administrar cargas de trabajo de procesamiento de datos. Es la forma más rápida y fácil de ejecutar Apache Hive, Spark y Hadoop en la plataforma de Google cloud. Se puede rápidamente crear un cluster mediante el aprovsionamiento de VM, comnfigurarlo y ejecutar las tareas de Hive o Spark, y finalmente eliminarlo para que no genere ningún coste adicional
-	Google Storage: En lugar de almacenar los datos en HDFS (sistema de ficheros distribuidos de Hadoop), se hará uso del almacenamiento proporcionado por Google (GCS), con el objetivo de desacoplar el almacenamiento del computo, y por lo tanto necesitar desplegar el cluster de Hadoop solamente cuando se tengan que ejecutar los   analisis de los datos con tecnologías como Hive o Spark.
Como se puede apreciar en el diagrama el despliegue del cluster y la ejecución de las tareas de computo serán ejecutadas a través de Cloud functions y activadas desde el servicio Cloud Scheduler.
Visualización
En una primera fase, los resultados obtenidos se publicaran a través de la herramienta de colaboración **Slack**. Se trata de una forma simple en la que varios usuarios pueden unirse a un proyecto de Slack y mediante sus clientes móvil o web recibir las recomendaciones generadas por nuestra aplicación.
En otra fase posterior, se podría mantener el cluster activo de forma que usuarios más avanzados pudieran hacer sus propios analisis y consultas  a través de los datos almacenados en el datalake o incluso hacer uso de herramientas de exploración y análisis de datos como Microsoft Power BI, Tableau o Superset para consultar los datos de una forma más amigable.
## Fase 2 (boceto)
Como una seguna fase, se presenta un un boceto,  de cómo mediante tecnologías de la plataforma GCP, podrían añadirse nuevos servicios y análisis a la arquitectura planteada en un principio.
![](images/arch-f2.png)
Como se muestra en el diagrama, se podría recopilar la información de redes sociales, y llevar a cabo analisis de sentimiento mediante tecnologías como Apache Spark. Del mismo modo también se podrían recoger los datos de los logs generados por los usuarios que interactúan con nuestro sistema, recogiendo comportamientos, valoraciones, etc. Todos estos datos podrías ser luego utilizados para enriquecer nuestro algoritmo de recomendaciones
Los resultados obtenidos de las recomendaciones y de análisis futuros se podrían ofrecer a los usuarios para que puedan ser consultados en cualquier momento. Si pensamos que estas tablas generadas no almacenarán gran cantidad de datos, estamos hablando de unos cuantos millones de registros y que se deberían poder consultar al mismo tiempo que se actualizan los resultados se podría utilizar una base de datos relacional como Postgresql/Mysql. Para ello GCP ofrece bases de datos gestionadas a través del servicio Google Cloud SQL.
Apache Kafka se añadiría como una capa que nos desacople la ingestión de los datos del procesado, de modo que al ser una tecnología de colas distribuidas y permanentes nos permita procesar la misma información por diferentes módulos o en instantes de tiempo distintos, en el caso de que sea necesario. A través de consumidores de Spark streaming se podrían se podrían ir procesando y/o almacenando los mensajes que llegarían a los distintos topics.
Bloque de ML, son los encargados de realizar analisis de Machine Learning mediante python y librerias como Scikit-learn. Después de llevar a cabo el estudio del modelo de ML adecuado a nuestra solución, máquinas virtuales podrían desplegar los modelos ML para llevar a cabo las tareas de predicción. Un ejemplo en nuetro sistema sería llevar a cabo un algoritmo no supervisado de clustering como es K-means con el que hacer una agrupación de viviendas en base a unas características, de modo que un usuario podría obtener recomendaciones que el sistema habría clasificado como similares a la seleccionada por el usuario.
Tambien se podría llevar a cabo recomendaciones haciendo uso de técnicas de filtrado colaborativo a través de Spark y la librería ALS, es decir hacer recomendaciones a partir de los gustos de los usuarios afines.
Bloque de monitorización. Debido a que ya estaríamos hablando de un sistema más complejo, sería recomendable tener una solución de monitorización mediante la que se puedan observar es estado de los metricas de interés como logs y datasets procesados, tamaño de las tablas de nuestro sistema, tiempo de procesados, etc. Para ello una buena solución sería montar un dahsbord mediante la herramienta Grafana, que permite crearnos nuestros scripts de python con los que obtener las metricas a monitorizar y almecenarlas en una base de datos enfocada a datos de series temporales como InfluxDB.


## Flujo de trabajo
Debido a la naturarelza de los datos en esta primera fase, las recomendaciones podrían ser diarias o semanales.  A continuación, se muestra un diagrama con el flujo de trabajo y datos que se ejecutará que se llevará a cabo para poder generar el analisis de forma programada.
![img](images/workflow-f1.png)

1.	A partir de los barrios de Madrid en formato geojson, se ha generado un csv con el que poder llevar a cabo nuestro análisis. Este csv se cargará, en la fase de despliegue, en el directorio _gs://kc-airbnb/datasets/_ creado para almacenar todos los datasets.
2.	El resto de datasets se extraerán mediante 3 programas en python alojados en Cloud Functions. Se trata de 3 cloud fucntions que ejecutan la recuperación de los datos y los almacenan en el directorio _gs://kc-airbnb/datasets/_ en formato csv. Estas funciones se ejecutarán todas las noches de forma automática, mediante su programación con Google Scheduler.
3.	Se trata del programa encargado de realizar el analisis y recomendaciones de los alquileres de Airbnb. Este programa también es ejecutado de forma automática mediante una cloud function programada. Este programa se compone de varias fases:
a.	Despliege de un cluster hadoop con el que poder hacer los analisis.
b.	Lanzar el análisis de las recomendaciones a través de unas consultas en Hive. Primero se cargarán las los datasets disponibles en tablas Hive y a continuación se ejecutarán las consultas necesarias (4).
c.	El resultado obtenido se volcará en una tabla y en el directorio _gs://kc-airbnb/results/recommended_listings/_ para que pueda con ser consultado a posteriori (5).
d.	Una vez finalizado el análisis, una función se encargará de leer el resultado y enviarlo a través de slack al canal configurado (6).
4.	Para evitar cargos adicionales se hará uso de la funcionalidad de eliminación programada de clústeres de Cloud Dataproc. Esta característica proporciona la opción de borrar un cluster después de un periodo de inactividad.
## Desarrollo
...
### Scripts 
#### Datasets
Los programas desarrollados para la extracción de los dataset se encuentran disponibles en el directorio scripts de este repositorio.
**get_airbnb_data.py**: Funciones python para descarga del dataset de Airbnb y preprocesado del csv para eliminar los caracteres de ‘new line’ que hay incluidos dentro de algunas descripciones del csv, ya que Hive no los puede manejar de forma correcta.
...
**get_poi_data.py**: Función con la API de Foursquare para recuperar los 50 lugares (puntos de interés) mejor valorados en cada uno de los barrios de Madrid. A partir del dataset neighbourhood.csv, cargado en la fase de despliegue, tenemos el centroide de cada barrio y será este punto el que se utilizará para buscar los puntos de interés recomendados por Foursquare.
Para poder utilizar la API de Foursquare de una manera mas eficiente, se ha optado por hacer las busquedas solamente en los centroides calculados de cada barrio.
#### Procesado
...
#### Hive sql
Se han generado dos scripts para procesar mediante Hive las recomendaciones de Airbnb.
...
o	La primera consulta calcula el número de eventos por alquiler que se encuentran en su barrio y a una distancia  max de 5 km.
o	La segunda consulta calcula Top de alquileres con mayor oferta de lugares de ocio recomendados, teniendo en cuenta que estén a menos de 2 km,  pertenezcan a su barrio y además tengan más de 2 eventos programados para los próximos días. Finalmente se ordenan las recomendaciones por numero de lugares recomendados y número de eventos próximos.

A continación se muestra una imagen con el cliente SQL (DBeaver) utilizado para realizar las pruebas sobre Hive.
...
La mayoría de tareas se podrían automatizar mediante el cliente gcloud ofrecido por GCP, pero en este proyecto se han realizado mediante scripts de python o haciendo uso del Google Cloud Console y su interfaz gráfica para administrar los recursos.
...
![img storage](images/gs-create-service-count-key.png)
...
-	Crear el bucket con nombre kc-airbnb
...
Cloud Functions
El despliegue de las cuatro funciones se ha realizado mediante la interfaz gráfica. Una mejora sería automatizar el despliegue de las cloud functions mediante la herramienta **gcloud** y empaquetando el código y los requirements.txt necesarios en un directorio del cloud storage.
A continuación se muestra las cloud functions desplegadas en el proyecto de GCP
![](images/cloud-functions.png)
Cloud Scheduler
Se desplegarán 4 programadores, encargados de llamar a las cloud functions anteriores a la hora programada. Este despliegue se ha ejecutado mediante comandos gcloud
‘’’
...
![img storage](images/cloud-schedulers.png)

## Exploracion de datos
En el directorio se muestra el notebook utilizado para llevar a cabo las pruebas y la exploración de datos de los datasets de Foursquare y eventos.
